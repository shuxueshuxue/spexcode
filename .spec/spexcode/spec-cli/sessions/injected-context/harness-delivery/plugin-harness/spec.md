...
status: active
hue: 280
desc: The plugin BUNDLE emitter — render the whole SpexCode system into one self-contained Claude-plugin bundle dropped into the spexcode.json-named folder, so a plugin host (z-code/Claude/future Codex) reads it natively with zero --append-system-prompt.
code:
