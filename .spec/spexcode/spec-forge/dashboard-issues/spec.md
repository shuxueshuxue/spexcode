...
status: active
hue: 280
desc: Surfaces each node's bound issues on the dashboard — lean open/closed counts and open identity on the graph, with actual rows demand-paged from the resident unified Issue snapshot.
code:
