...
每个节点可以挂场景(描述 + 预期),用真实浏览器/真实 API 跑出读数,读数进 git,截图/视频按内容寻址存在仓库旁的证据缓存里(像素刻意不进 git 历史)。修 bug 的纪律是 A/B:先跑出一条**失败读数**复现它,修完再跑出**通过读数**——一个 bug 的完整生命周期在时间线上可回放。这个仓库目前有 750 条这样的读数。
