...
spex serve               # 当前项目的后端;自动注册到当前用户
# 另开一个 shell,每个用户只运行一次:
spex dashboard           # 共享的 host gateway + UI,:5173
```

每个要上线的项目都在自己的仓库里运行 `spex serve`;新增项目可以用
`spex serve --port 8788` 这样的空闲后端端口。每个后端都会自行注册到当前用户的 host registry。
唯一的 `spex dashboard` 会自动发现已经运行和之后启动的后端,通过全局 `/projects` 完成
项目切换和管理,并把每个项目的看板放在 `/p/:id/` 下。不再为每个项目启动单独的看板进程,
也不再配对 API/UI 端口。

`--harness` 选择要 materialize 的 agent 集成(`codex` 是另一个内置选项)。`spex init` 是增量式的,
在任何已有 git 仓库上可用,不会覆盖你的文件:生成根节点 `.spec/project/spec.md`、一份起始
`spexcode.json`,安装 git hooks,并写入所选 harness 的托管契约,让在这个仓库里工作的 agent
自己发现这套工作流。
