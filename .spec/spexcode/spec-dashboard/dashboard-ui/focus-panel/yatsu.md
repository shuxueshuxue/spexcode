...
scenarios:
  - name: panel-shows-issues-and-scenarios
    description: >-
      ...
      spec-cli). Look at the LEFT focus panel: it names the focused node, then a SCENARIOS section
    ...
    expected: >-
      The left panel renders the focused node's Scenarios (per-scenario state + expected, with a
...
screenshots the left panel showing that node's Issues and Scenarios side by side, filing it as a reading
