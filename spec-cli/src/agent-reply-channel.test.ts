...
test('sessionHeadline: with no name, the LIVE activity (Claude Code summary) wins over the prompt title — the fix', () => {
  assert.equal(deriveHeadline(sess({ activity: 'merging round-2 into main', promptPreview: 'fix the session title chain', title: 'fix the session' })), 'merging round-2 into main')
})
