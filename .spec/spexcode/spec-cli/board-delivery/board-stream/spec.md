...
**Four event sources, one debounced pipeline.** (1) A recursive `fs.watch` on the per-user session store
...
store dir) still lands, once per tick total instead of once per open dashboard. All four funnel into one
debounced fire that collapses a burst (a merge touches many records) into one signal.
