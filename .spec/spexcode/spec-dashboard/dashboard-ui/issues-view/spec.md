...
- **A human writes from here — to the issue's OWN store.** EVERY issue's detail carries a **reply
  ...
  **autocomplete dropdown** the console's inputs use ([[mentions]]'s shared menu — one implementation,
  ...
  affordance lives with the discussion actions near the composer at the end of the thread, never in the
  title's top-right chrome: closing is a lifecycle action on the conversation, not a way to dismiss the
