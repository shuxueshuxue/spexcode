...
scenarios:
  - name: ack-on-merge-commit
    description: >
      ...
      `spex spec ack <node> --reason "…"` while HEAD is that merge commit — no SPEXCODE_ALLOW_MAIN, no
      SPEXCODE_SKIP_LINT. Then run `spex spec lint` and check the node's drift.
    expected: >
      ...
      and `spex spec lint` no longer reports the acknowledged drift. A commit on the trunk that DOES
...
bash re-derivation of the guard) and the real `spex spec ack` / `spex spec lint` CLI, in a throwaway clone so
