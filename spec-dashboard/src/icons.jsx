...
// The icon-only button: label is REQUIRED and becomes both the native tooltip and the accessible
// name, so no icon button can ship without either. (The tooltip layer may later upgrade `title`
// to a richer component; the aria-label contract stays.)
export function IconButton({ icon, label, onClick, className, size, iconClassName, ...rest }) {
  return (
    <button type="button" className={className} title={label} aria-label={label} onClick={onClick} {...rest}>
