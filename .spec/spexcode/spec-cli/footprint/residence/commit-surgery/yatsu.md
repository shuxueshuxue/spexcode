...
scenarios:
  ...
  - name: git-native-anchors-only
    description: >
      ...
      lifecycle event through dispatch.sh, and read whether the rendered contract moved; (2) commit the
      same edit (pre-commit anchor) and switch branches (post-checkout anchor), reading the contract after
      each.
    expected: >
      (1) The harness event renders NOTHING — the dispatcher only dispatches; the contract file is
      byte-unchanged. (2) The commit's unconditional materialize and the post-checkout refresh each bring
      the rendered contract up to date — .config edits are git-transactional, taking effect at the git
