...
function IssueDetail({ issue: th, specs, sessions, onFocusNode, onWrite }) {
  ...
  return (
    <div className="fvd">
      ...
      <ReplyComposer id={th.id} specs={specs} sessions={sessions} focusId={nodes[0] || null} onDone={onWrite} />
    </div>
  )
}

// a small textarea + Send under an issue's detail — posts a reply as 'human' (the API routes it by the
// issue's store: forum commit or real forge comment) and reloads. An @-mention in the text summons a
// worker regardless of store; the returned outcomes string surfaces via onDone. The textarea carries the
// SAME `[[node]]`/`@session` autocomplete as the console ([[mentions]], one shared menu, never a fork);
// the composer is docked at the detail's bottom, so its menu opens UPWARD. The thread's first node leads
// the `[[` list — the convenient default topic here.
function ReplyComposer({ id, specs, sessions, focusId, onDone }) {
  const t = useT()
  const [body, setBody] = useState('')
  const [busy, setBusy] = useState(false)
  const [err, setErr] = useState('')       // a failed write (a forge can be unreachable) surfaces, never swallows
