...
scenarios:
  - name: nonweb-zero-match-guides
    description: >
      ...
      sourceExtensions (ts/tsx/js/jsx), run `spex lint`. Coverage finds zero source. Read the transcript.
    expected: >
  ...
  - name: forgiving-input-normalized
    description: >
      ...
      src/pkg/util.test.py. Run `spex lint` and read the coverage findings.
    expected: >
...
YATU through the real `spex lint` CLI, never by reading lint.ts. Stand up a throwaway git repo shaped
like a non-web adopter (a `.spec/project` node, `.py` sources, a `spexcode.json`), run `spex lint` with
`node spec-cli/bin/spex.mjs lint` from that repo's root, and read the emitted coverage transcript. The
