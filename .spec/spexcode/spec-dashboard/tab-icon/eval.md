...
scenarios:
  - name: configured-and-default
    ...
    expected: |
      With nothing configured, the tab shows the default SpexCode mark (a terminal-style pixel-block
      letter S in ANSI-style violet→fuchsia row steps on a dark terminal tile) shipped by index.html (the <link rel="icon"> resolves to that inline SVG data-URI). Setting dashboard.icon to an emoji
