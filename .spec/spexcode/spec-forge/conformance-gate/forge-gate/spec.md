...
The orchestration of the [[deliver-port]] **verdict driver** ([[conformance-gate]]) — an external PR
reaches it as a [[session-origin]] `pr` session, and this is the entrypoint that makes the gate run and the
verdict land.

The capstone of [[conformance-gate]] — the entrypoint that makes the gate *run* and the verdict *land*.
