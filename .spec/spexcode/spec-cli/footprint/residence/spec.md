...
`spex materialize` in its setup step; there is no committed-artifact delivery mode — no generator means not
using SpexCode.
...
**A contract file's residence is a live CONTENT fact, re-judged at every materialize** (never an install-time
...
setup. (The vote's literal field name was `render` — the word survives here only as that historical
identifier.) Deferred lanes that survive unchanged: the readings store move (evidence blobs already live in
