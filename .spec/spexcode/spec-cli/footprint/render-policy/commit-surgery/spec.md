...
status: active
hue: 200
desc: The git-native anchors of the footprint — pre-commit runs an UNCONDITIONAL materialize + staged-index repair (strip leaked blocks in place, evict HEAD-untracked artifacts; never reject), post-checkout/post-merge re-render; no harness event ever triggers a render.
code:
...
EVENT-anchored one: the render is provably fresh at the only moments freshness matters. History is written
at exactly one gate (commit), and a clone's render inputs (.spec/.config content, a contract file's
...
rendered files, never a trigger — the old dispatch-gate auto-materialize is retired, and `.config` edits
...
1. *Unconditional materialize.* Every machine-fixable staleness (exclude entries, filter binding, a
   ...
   precisely because of the vote's collapse: materialize's write surface is untracked renders + per-clone
...
2. *Staged-index surgery — repair, never reject.* Over staged ∩ artifact paths, by kind: a contract blob
   ...
   hook — a legacy committed render heals via the block-strip, converging history toward pristine without
...
with the risk — the renders and the hook are planted by the same materialize, so any clone that can leak
...
post-checkout re-render writes the block back into the working file, binds the filter, and withdraws the
...
a pure render must stay excluded or `??` noise and empty-file scooping return). Every other path — an
