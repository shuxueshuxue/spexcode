...
**Reviewing a change in place.** A node mid-change is near-empty on the board — most of all a freshly *added* **ghost**, which carries no committed title or version yet — so its overlay marks alone are too thin to review. Hovering or focusing such a node opens a small **session peek** that drops below it (the mirror of the issue card, which drops above, so the two never collide): the live session(s) changing this node, each rendered as the **same row the top-right window uses** (see [[session-console]]) — avatar, name, and an **op tally** (`+2 ~1 ✕1`) of how many nodes that session is changing. Clicking a row hands straight to the lock flow ([[session-console]] · [[keyboard-nav]]): the graph locks onto that session, the rest greys, and focus jumps to its first changed node. So a pending change is reviewable from the changed node itself, through the same session lens used everywhere else — a ghost included, which turns solid while you hover or focus it so its peek reads clearly.
