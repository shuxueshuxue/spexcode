...
try {
  ...
  assert.match(await onlineSwitcherItem.getAttribute('href') || '', /\/p\//, 'online project keeps its scoped URL')
  assert.equal(await onlineSwitcherItem.getAttribute('aria-disabled'), null, 'online project is not disabled')
  assert.match(await onlineSwitcherItem.textContent(), /online/, 'online state is visible in the switcher')
