...
scenarios:
  - name: full-width-without-sidebar
    ...
    description: >-
      ...
      graph page, and canvas rectangles; inspect the DOM for a persistent focus panel or graph-side resize
      divider; and capture the whole board.
    expected: >-
      The graph page and canvas consume the main area's full width and height, with no persistent focused-node
      sidebar, no graph resize divider, and no document horizontal overflow. Issues and eval details remain
      available on demand through their routed pages and the node-info popup.
    code:
