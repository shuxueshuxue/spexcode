...
cd your-repo
spex init --harness claude,codex,opencode,pi   # 生成 .spec/、安装 hooks、写入 agent 契约
```
...
`spex doctor` 是需要主动调用的只读健康诊断。它的 altitude 检查会报告像实现细节堆的正文,并给出依据和修复方向;
这类启发式判断不进入 lint 或提交门禁。
